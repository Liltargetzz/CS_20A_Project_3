...
namespace arraybased {
	...
	public:
		...
		void printItems() const {
			cout << "--------------------------------" << endl;
			cout << "Addr     Size\tFunc\tLine" << endl;
			if (m_numEntries == 0)
				cout << "*** No Memory Tracked ***" << endl;
		...
		void	add(void* itm, char const* file, char const* func, int line, std::size_t sz) {
			if (m_numEntries < MAX_ALLOC_TRACK) {
				m_dispatched[m_numEntries].addr = itm;
		...
		bool	deleteItem(void* itm) {
			int index = -1;
			for (int i = 0; i < m_numEntries; i++) {
				if (m_dispatched[i].addr == itm) {
					index = i;
					break;
				}
			}
...
/*
void removeObjTrace(void *ptr) {
	g_allocList.deleteItem(ptr);
}
void dbgDelete(void *ptr) {
	g_allocList.deleteItem(ptr);
	std::free(ptr);
}
*/